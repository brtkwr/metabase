...
(deftest resolve-personal-owner-id-test
  (testing "resolve-personal-owner-id correctly resolves personal collection ownership"
    (let [user1-id               (mt/user->id :rasta)
      ...
      (mt/with-temp
        ...
        (testing "nil collection-id returns nil"
          (is (nil? (#'semantic.index/resolve-personal-owner-id nil))))

        (testing "shared collection returns nil"
          (is (nil? (#'semantic.index/resolve-personal-owner-id shared-coll-id))))

        (testing "personal collection returns its owner"
          (is (= user1-id (#'semantic.index/resolve-personal-owner-id user1-personal-coll-id)))
          (is (= user2-id (#'semantic.index/resolve-personal-owner-id user2-personal-coll-id))))

        (testing "sub-collection of personal collection returns root personal owner"
          (is (= user1-id (#'semantic.index/resolve-personal-owner-id user1-sub-coll-id)))
          (is (= user2-id (#'semantic.index/resolve-personal-owner-id user2-sub-coll-id))))

        (testing "caching works correctly"
          (binding [semantic.index/*personal-owner-id-cache* (atom {})]
            (is (= user1-id (#'semantic.index/resolve-personal-owner-id user1-personal-coll-id)))
            (is (= {user1-personal-coll-id user1-id} @semantic.index/*personal-owner-id-cache*))
